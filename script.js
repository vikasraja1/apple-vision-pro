...
function canvas() {
  ...
  // Fill the imageUrls array with URLs of the images
  for (let i = 0; i <= 144; i++) {
    const imageUrl = `https://www.apple.com/105/media/us/apple-vision-pro/2023/7e268c13-eb22-493d-a860-f0637bacb569/anim/360/large/${String(i).padStart(4, '0')}.jpg`;
    imageUrls.push(imageUrl);
  }

  // Preload images and start animation when all images are loaded
  let imagesLoaded = 0;
  ...
  window.addEventListener("resize", function () {
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
    // Restart the animation when the canvas is resized
    animate();
  });
}

// Call the canvas function to initialize the animation
canvas();
